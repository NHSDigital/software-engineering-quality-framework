...
- **Minimise access** to production
  ...
  - Frequently asked questions:
    - Q: If I can't access production, how can I check data, for example to respond to a support call? A: one approach is to build a facility to clone the production database into a short-lived and isolated copy, so that data can be checked safely without anyone accessing production. Read-replicas can potentially be used instead, but they are (obviously) limited to read-only, and will consume more cost and energy than on-demand clones. As above, access must be audited and strictly controlled. 
